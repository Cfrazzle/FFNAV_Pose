...
Truth_3D_plot   = 0;        % 3D Positions from Truth Orbit Propagation
...
if EKF_xd_plot == 1
    figure(fig_id); fig_id = fig_id + 1; hold on;
    plot(time_plot, EKF_output(:,6)*10^3, '-')
    plot(time_plot, NERM_output(:,6)*10^3, '-.')
    plot(time_plot, GPS_output(:,5)*10^3, '--')
    plot(time_plot(2:end), diff(r_clean(:,1))*10^3, '-x')
    title('EKF Output X-dot vs Time')
    xlab = xlabel('Time (hrs)');
    ylabel('X_d_o_t (mm/s)')
    set(gcf, 'units','normalized','outerposition',[0 0 1 1]);
    legend('EKF Output', 'Dynamics', 'GPS Measurement','Calculated via diff()')
    grid on  
end
...
if EKF_yd_plot == 1
    ...
    plot(time_plot(2:end), diff(r_clean(:,2))*10^3, '-x')
...
if EKF_zd_plot == 1
    ...
    hold on
    plot(time_plot(2:end), diff(r_clean(:,3))*10^3, '-x')
...
if EKF_thetad_plot == 1
    ...
    plot(time_plot(2:end), diff(theta_clean), '-x')
    title('EKF Output \theta-dot vs Time')
    xlab = xlabel('Time (hrs)');
    ylabel('\theta_dot (rad/s)')
    set(gcf, 'units','normalized','outerposition',[0 0 1 1]);
    legend('EKF Output', 'Dynamics', 'Calculated via diff()')
    grid on  
end
...
if EKF_rtd_plot == 1
    ...
    plot(time_plot(2:end), diff(rt_clean)*10^3, '-x')
